...
export class CalendarQueue {
  ...
  async handleCalendarJob(job: Job<CalendarJobPayload>) {
    ...
    const eventMeta = await this.calendarService.generateCalendarEvent(
      task,
      context,
      deadline,
    );
    if (!eventMeta) {
      this.logger.warn(
        `Could not generate calendar event metadata for task: "${task}"`,
      );
      return;
    }
    ...
    if (parsed) {
      const range = this.dateTimeParser.computeRange(
        parsed.startISO,
        eventMeta.suggestedDuration,
      );
      from = range.from;
      to = range.to;
      this.logger.log(
      ...
      this.logger.warn(
        `Could not parse date from task — defaulting to tomorrow 10 AM`,
      );
      from = new Date();
    ...
    if (result) {
      this.logger.log(
        `Calendar event "${eventMeta.title}" created: ${result.id}`,
      );
