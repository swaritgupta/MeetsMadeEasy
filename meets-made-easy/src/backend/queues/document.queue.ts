...
export class DocumentQueue{
  ...
  async handleDocumentJob(job: Job<DocumentJobData>){
    this.logger.log(`Document job is being processed for task: "${job.data.task}"`);
    ...
    if(doc){
      ...
      const createdDoc = await this.documentService.createDocument(doc, meetingId);
      if (createdDoc?.documentId) {
        this.logger.log(`Google Document successfully created with ID: ${createdDoc.documentId}`);
      }
      await this.documentService.saveDocument(doc, meetingId, task);
    } else {
      this.logger.warn(`Failed to generate document content for task: "${task}"`);
    }
  }
}